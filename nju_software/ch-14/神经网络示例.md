...
$$
\begin{aligned}
\text{原始空间} & \xrightarrow{\text{tanh}(W_1\boldsymbol{x} + \boldsymbol{b}_1)} \text{隐藏空间} \\
& \xrightarrow{\sigma(W_2\boldsymbol{h} + \boldsymbol{b}_2)} \text{输出空间}
\end{aligned}
$$
...
1. **隐藏层变换**：
   ...
   - $\boldsymbol{X}$：输入矩阵（4×2）
   - $\boldsymbol{W}^{(1)}$：权重矩阵（2×4）
...
2. **输出层计算**：
   ...
   \hat{\boldsymbol{y}} &= \sigma(\boldsymbol{z}^{(2)})
   \end{aligned}
   $$

   - $\sigma$：Sigmoid 函数，将输出压缩到[0,1]区间
   - $\hat{\boldsymbol{y}}$：最终预测概率
...
1. **输出层梯度**：
   ...
   $$
   \begin{aligned}
   \frac{\partial \mathcal{L}}{\partial \boldsymbol{z}^{(2)}} &= \frac{1}{N} (\hat{\boldsymbol{y}} - \boldsymbol{y}) \\
   \frac{\partial \mathcal{L}}{\partial \boldsymbol{W}^{(2)}} &= (\boldsymbol{a}^{(1)})^\top \frac{\partial \mathcal{L}}{\partial \boldsymbol{z}^{(2)}} \\
   \frac{\partial \mathcal{L}}{\partial \boldsymbol{b}^{(2)}} &= \text{sum}(\frac{\partial \mathcal{L}}{\partial \boldsymbol{z}^{(2)}}, \text{axis}=0)
   \end{aligned}
   $$
...
2. **隐藏层梯度**：
   ...
   $$
   \begin{aligned}
   \frac{\partial \mathcal{L}}{\partial \boldsymbol{z}^{(1)}} &= \left( \frac{\partial \mathcal{L}}{\partial \boldsymbol{z}^{(2)}} (\boldsymbol{W}^{(2)})^\top \right) \odot (1 - (\boldsymbol{a}^{(1)})^2) \\
   \frac{\partial \mathcal{L}}{\partial \boldsymbol{W}^{(1)}} &= \boldsymbol{X}^\top \frac{\partial \mathcal{L}}{\partial \boldsymbol{z}^{(1)}} \\
   \frac{\partial \mathcal{L}}{\partial \boldsymbol{b}^{(1)}} &= \text{sum}(\frac{\partial \mathcal{L}}{\partial \boldsymbol{z}^{(1)}}, \text{axis}=0)
   \end{aligned}
   $$
