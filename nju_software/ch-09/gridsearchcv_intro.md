...
在机器学习中，模型的性能很大程度上取决于超参数的选择。超参数是那些在模型训练之前需要手动设置的参数，而不是通过数据学习得到的参数。对于 `KNN`（`K-近邻`）算法来说，一个关键的超是参数 `n_neighbors`（邻居数量），它直接影响模型的泛化能力和预测性能。本文将介绍如何使用 `sklearn` 库中的 `GridSearchCV` 来优化 `KNN` 模型的超参数，从而提高模型的性能。
